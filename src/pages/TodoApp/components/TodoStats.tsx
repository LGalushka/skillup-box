...
export const TodoStats = memo(({ stats }: TodoStatsProps) => {
  return (
    <section className="grid grid-cols-3 gap-4">
      ...
      <StatCard
        ...
        colorClass={stats.active === 0 ? 'text-text-light' : 'text-primary'}
      />
      <StatCard
        label="Готово"
        value={`${stats.completed} (${stats.percentCompleted}%)`}
        colorClass={stats.completed === 0 ? 'text-text-light' : 'text-primary'}
      />
    </section>
  );
});
